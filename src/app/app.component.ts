...
export class AppComponent implements OnInit, AfterViewChecked {
	...
	ngAfterViewChecked() {
		this.textWindow.nativeElement.scrollTop = 1000000; //this.textWindow.nativeElement.offsetHeight + 100;
	}

	public extractWordData(text) {
	...
	private getAliases(text) {
		let match = text.match(/{(.*)\|(.*)}/);

		while(match) {
			this.aliases.push({display: match[1], content: match[2]});

			text = text.replace(match[0], `\{${this.aliases.length - 1}\}`);
			match = text.match(/{(.*)\|(.*)}/);
		}
	...
	private getKeywords(gameState) {
		this.keywords = gameState.world.settings.keywords.map(x => x.keyword);
	}
}
